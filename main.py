...
print('Phyphox Acceleration Analyser v2.0.1\nby HanZero\n')
print('输入序号进入相应模式：\n0 - 实时监控\n1 - 数据分析\n')
choice = input('>>> ')
if choice == '0':
    # 请求IP
    ...
    while is_recording:
        sleep(refresh_time)
        # 重置基线校正后和滤波后数据序列
        corrected_x = []  # 基线校正后x方向加速度数据序列
        corrected_y = []
        corrected_z = []
        data_x = []  # 滤波后加速度x方向数据序列
        data_y = []
        data_z = []
        data_a = []
        vx = []  # 滤波后速度
        ...
        for x,y,z in zip(raw_data_x,raw_data_y,raw_data_z):
            ...
            corrected_z.append(z)

        # 转换为速度
        corrected_vx = (np.cumsum(corrected_x)) * (1/sampling_rate) + last_latest_vx
        corrected_vy = (np.cumsum(corrected_y)) * (1/sampling_rate) + last_latest_vy
        corrected_vz = (np.cumsum(corrected_z)) * (1/sampling_rate) + last_latest_vz
        ...
        # 对原三方向加速度和速度进行滤波
        # 加速度
        for row in filter_wave(x=corrected_x):
            data_x.append(row)
        for row in filter_wave(x=corrected_y):
            data_y.append(row)
        for row in filter_wave(x=corrected_z):
            data_z.append(row)
        # 速度
        ...
        for x,y,z in zip(data_x,data_y,data_z):
            data_a.append(sqrt(x**2+y**2+z**2))
        # 速度
        for x,y,z in zip(vx,vy,vz):
            va.append(sqrt(x**2+y**2+z**2))

        # 存储处理后数据
        save_processed_data(start_time, last_latest_time, (raw_data_t,data_x,data_y,data_z,data_a), 'a')
        ...
        if rt_a_max > a_max:
            a_max = rt_a_max
            a_max_time = round(raw_data_t[data_a.index(a_max)], 2)
        # 计算PGV
        rt_v_max = max(va)
        if rt_v_max > v_max:
            v_max = rt_v_max
        ...
        #计算烈度
        rt_ia_csis,rt_i_csis = csis_calc(rt_a_max,rt_v_max)
    ...
    if not processed:
        with open(path+'/Raw Data.csv', 'r') as raw_data_file:  # 解析原数据
            ...
            # 转换为速度
            corrected_vx = np.cumsum(corrected_x) + last_latest_vx
            corrected_vy = np.cumsum(corrected_y) + last_latest_vy
            corrected_vz = np.cumsum(corrected_z) + last_latest_vz
        ...
        with open(path+'/Processed Data (Linear Acceleration).csv', 'r') as data_file:  # 解析a数据
            ...
            for row in data:
                if row[0] == 'Time (s)':
                    continue
                raw_data_t.append(float(row[0]))
                data_x.append(float(row[1]))
                data_y.append(float(row[2]))
                data_z.append(float(row[3]))
                data_a.append(float(row[4]))
    ...
    # 计算烈度
    ia_csis, i_csis = csis_calc(a_max, v_max)
    ia_jma, i_jma = jma_calc(a_max)
